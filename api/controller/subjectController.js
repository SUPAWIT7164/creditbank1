...
const buddhistEra = require("dayjs/plugin/buddhistEra"); // โหลด plugin แปลงปีเป็น พ.ศ.
...
const generateBilling = async(req, res, next) => {
    ...
    try {
        ...
        // doc.image("assets/รายเซ็นอาจารย์ไม่มีพื้น.png", 20, 10, {
...
const handleExpdf = async(req, res) => {
    ...
    try {
        ...
        const thSarabunPSKFont = fs.readFileSync("fonts/THSarabunNew.ttf");
        const thSarabunPSKFontBuffer = Buffer.from(thSarabunPSKFont);
        dayjs.extend(buddhistEra); // ขยาย Day.js ด้วย plugin
        const dateString = result[0].CREATE_DATE; // ดึงข้อมูลวันที่จากผลลัพธ์
        const thaiDate = dayjs(dateString).locale("th"); // แปลงวันที่เป็น object และตั้งค่า locale เป็นภาษาไทย
        const formattedThaiDate = dayjs(thaiDate).format("DD เดือน MMMM พ.ศ. BBBB"); // แปลงรูปแบบวันที่เป็น dd/mm/พ.ศ.
        console.log(formattedThaiDate);
        const doc = new Document({
            size: "A4",
            margins: {
                top: 20,
